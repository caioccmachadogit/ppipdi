...
public class AsyncReceberCombo extends AsyncTask<String, String, String>{
	private ProgressDialog pd;
	private Activity activity;
	private TarefaInterface ti;
	
	public AsyncReceberCombo(Activity activity, TarefaInterface ti){
	...
	protected String doInBackground(String... params) {
		...
		try {
			...
			String codConfig = params[1];
			publishProgress("Verificando Combos");
			WSCombo wsCombo = new WSCombo();
			json = wsCombo.ReceberCombo(filtro, codConfig);
		}
		catch (Exception e) {
	...
	protected void onPostExecute(String result) 
	{
		...
		MainActivity.COUNTSITE = 0;
		MainActivity.COUNTCOMBO++;
